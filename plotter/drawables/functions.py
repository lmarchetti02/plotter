...
class FunctionPlot(Drawable):
    ...
    Attributes:
        ...
        wider (tuple[float, float], optional): The percentages (left, right) to which
            the domain `x` is to be widened. Defaults to `(0, 0)`.
        dens (int, optional): The density factor to be passed to `_make_wider()`.
            Defaults to 1.
        y (NArray1D[Any] or None): The values of `f` evaluated on the (possibly widened)
            domain, populated after `__post_init__` runs.
    """
    ...
    def __post_init__(self) -> None:
        """Widens/densifies the domain and evaluates `f` on it."""
    ...
    def draw(self, canvas: Canvas | ZoomInset, plot_n: int = 0, label: str | None = None, **kwargs) -> None:
        ...
        logger.info("Called 'FunctionPlot.draw()'")

        LinePlot(self.x, self.y).draw(canvas, plot_n, label, **kwargs)
    ...
    def _make_wider(data: NArray1D[Any], left: float, right: float, density: int) -> NArray1D[Any]:
        ...
        return FunctionPlot._make_denser(wider_data, density)
