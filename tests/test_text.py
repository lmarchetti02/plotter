...
from plotter.drawables.drawable import Drawable
from plotter.helpers.text import PlotText, Text


class TestGetEmpyText:
    """Tests for `PlotText.get_empy_text`."""

    def test_returns_a_complete_placeholder(self) -> None:
...
class TestReadJson:
    """Tests for `Text.read_json`."""

    def test_reads_json_with_or_without_extension(
        self,
        workspace: Path,
        text_file: Path,
    ...
    def test_creates_a_default_file_when_json_is_missing(self, workspace: Path) -> None:
    ...
    def test_falls_back_to_empty_text_for_unparseable_json(self, workspace: Path, payload: str) -> None:
    ...
    def test_raises_when_json_contains_the_wrong_number_of_subplots(
        self,
        workspace: Path,
        sample_text_data: list[dict[str, object]],
    ) -> None:
